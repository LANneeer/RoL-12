...
from states.mainStates import TeacherRegister, StudentsRegister


# /start с диплинком (для учителей)
@dp.message_handler(CommandStart(deep_link='spc015523'), Ban())
async def start_how_teacher(message: Message):
    if not await db.exist_user(id=message.from_user.id):
        if message.from_user.username is None:
            await message.answer(_('<b>У вас нету имени пользователя!</b>\n'
            ...
            await message.answer(_('<b>Здравствуйте, вас приветствует ROL-bot</b>\n\n'
        ...
        await message.answer(_('<b>Вы есть в базе данных!</b>\n'
                               'Нажите "назад":'),
                             reply_markup=back)
        await message.delete()
...
async def start_how_student(message: Message):
    if not await db.exist_user(id=message.from_user.id):
        if message.from_user.username is None:
            await message.answer(_('<b>У вас нету имени пользователя!</b>\n'
                                   '<b>Поставьте себе имя пользователя что-бы пройти регистрацию</b>\n\n'
