...
from keyboards.default.Menu_buttons import back_and_send
from keyboards.default.Subjects import subjects_list
from loader import dp, db, bot
from state import StudentsState, StudentsUpdates
from utils.misc import rate_limit


# FUNC СДАТЬ ДОМАШНЕЕ ЗАДАНИЕ
@dp.message_handler(
    lambda message: message.text and message.text.upper() in ['СДАТЬ ЗАДАНИЕ', 'СДАТЬ ЗАДАНИЕ 📤'],
    Students())
async def send_hw(message: Message):
    await message.answer('Выберите нужный вам <b>предмет</b>:', reply_markup=Subject_buttons)
...
async def select_subject(message: Message, state: FSMContext):
    ...
    if subject == '🔙НАЗАД' or subject == 'НАЗАД':
        ...
        if subject in subjects_list:
            ...
            await message.answer('Отправьте вашу работу в виде <b>фото или документа</b>:', reply_markup=back_and_send)
            await StudentsState.next()
        else:
            await message.answer('Нету такого предмета! Введите с клавиатуры!', reply_markup=Subject_buttons)
...
async def send_doc(message: Message, state: FSMContext):
    ...
    if answer == 'СДАТЬ':
        ...
        if len(temp_data) != 0:
            ...
            await state.finish()
            await message.answer(f'Задание по предмету <b>{subject}</b>, отправлено!\n', reply_markup=Students_buttons)
        else:
        ...
        await state.finish()
        await message.answer('<b>Вы отменили отправку работы!</b>')
        await message.answer(text='<b>Главное меню</b>\n'
                                  'выберите действие с кнопок ниже\n\n'
                                  'Доп. Информация: /help',
                             reply_markup=Students_buttons)
...
@dp.message_handler(
    lambda message: message.text and message.text.upper() in ['УЗНАТЬ ЗАДАНИЕ', 'УЗНАТЬ ЗАДАНИЕ 📥'],
    Students())
    ...
    if subject == '🔙НАЗАД' or subject == 'НАЗАД':
        ...
        if subject in subjects_list:
            ...
            if bool(len(file)):
                ...
                for file in file:
                    if temp_var != file[1]:
                        await message.answer('*==============================*\n\n'
                                             f'Учитель по предмету: {subject.title()}\n'
                                             f'Аккаунт: @{file[2]}', reply_markup=Students_buttons)
                        temp_var = file[1]
                    try:
                ...
                await message.answer(f'Задания по предмету <b>{subject.title()}</b> не заданы!',
                                     reply_markup=Students_buttons)
            await state.finish()
        else:
            await message.answer('Нету такого предмета! Введите с клавиатуры!', reply_markup=Subject_buttons)
...
# FUNC ЗАДАТЬ ВОПРОС
@dp.message_handler(
    lambda message: message.text and message.text.upper() in ['ВОПРОС К УЧИТЕЛЮ', 'ВОПРОС К УЧИТЕЛЮ ℹ️'],
    Students())
async def question(message: Message):
    ...
    send = '<b>Нажмите на <i>@</i> и начните переписку!</b>\n\n'
    for data in teachers_username:
        send += f'Учитель по предмету <b>{data[1]}</b>: @{data[0]}\n'
    await message.answer(text=send, reply_markup=Students_buttons)


# FUNC ОТМЕТИТЬСЯ НА УРОКЕ
@dp.message_handler(
    lambda message: message.text and message.text.upper() in ['ОТМЕТИТЬСЯ НА УРОКЕ', 'ОТМЕТИТЬСЯ НА УРОКЕ ❗️'],
    Students())
async def check_hw(message: Message):
    await message.answer('Выберите нужный вам <b>предмет</b>:', reply_markup=Subject_buttons)
        ...
        if subject in subjects_list:
            ...
            if await db.exist_open_accepts(school=school, group=student[2], classroom_number=student[0],
                ...
                if accepts is not None:
                    if accepts:
                        await message.answer('Вы уже отмечены', reply_markup=Students_buttons)
                    else:
                        await db.update_students_accepts(id=message.from_user.id, subject=subject, accept=True)
                        await message.answer(f'Вы успешно отметились по предмету <b>{subject}</b>',
                                             reply_markup=Students_buttons)
                else:
                ...
                await message.answer(f'Урок по предмету <b>{subject}</b> не начался', reply_markup=Students_buttons)
                await state.finish()
        else:
            await message.answer('Нету такого предмета! Введите с клавиатуры!', reply_markup=Subject_buttons)
